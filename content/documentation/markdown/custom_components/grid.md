...
A responsive auto-fill grid. Use `cols:2` – `cols:5` for a fixed column count, or leave it out for an auto-responsive layout.
...
Without a `cols:` parameter it auto-fills columns based on available space (minimum 280 px per column). With `cols:N` it locks to exactly N columns on desktop. It works best with `[card:]` or `[box:]` children of similar heights. Always collapses to one column on mobile (see the responsive behavior below).
