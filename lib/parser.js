 ...
 *   [collector: url; tags:bool; date:bool; search:bool; arrange:cols|grid|rows; display_items:int|all] — Content collector
 *   [start:item] / [end:item]    — Item in collector (has metadata)
 */
...
function parseBlocks(body, opts = {}) {
  ...
  while (i < lines.length) {
        ...
        if (blockAction === "start") {
          ...
          while (i < lines.length) {
            ...
            if (
              ...
              i++; // consume closing [end:item]
              break;
            }
            // Parse metadata lines (e.g., @date: 2024-01-15, @tags: tag1, tag2)
            const metaMatch = lines[i].match(/^@(\w+):\s*(.+)$/);
            if (metaMatch) {
              const key = metaMatch[1];
              const value = metaMatch[2].trim();
              if (key === "tags") {
                itemMetadata.tags = value.split(",").map((t) => t.trim());
              } else {
          ...
          html += `<div class="item" data-metadata='${JSON.stringify(itemMetadata)}'>${parseBlocks(itemContent).html}</div>`;
          continue;
        }
      }
      continue;
    ...
    // ── Collector blocks (reads from filesystem) ──────────────────────────────
    // [collector -> url; tags:true; date:true; search:true; arrange:cols|grid|rows; display_items:5]
    const collectorMatch = line.match(/^\[collector\s*->\s*([^;\]]+)(.*?)\]$/);
    if (collectorMatch) {
      const collectorPath = collectorMatch[1].trim();
      const params = collectorMatch[2] || "";
      ...
      const tagMatch = params.match(/tags:\s*(true|false)/i);
      if (tagMatch) collectorOpts.tags = tagMatch[1].toLowerCase() === "true";

      const dateMatch = params.match(/date:\s*(true|false)/i);
      if (dateMatch) collectorOpts.date = dateMatch[1].toLowerCase() === "true";

      const searchMatch = params.match(/search:\s*(true|false)/i);
      if (searchMatch)
        collectorOpts.search = searchMatch[1].toLowerCase() === "true";

      const arrangeMatch = params.match(/arrange:\s*(cols|grid|rows)/i);
      if (arrangeMatch) collectorOpts.arrange = arrangeMatch[1].toLowerCase();

      const displayMatch = params.match(/display_items:\s*(all|\d+)/i);
      if (displayMatch) collectorOpts.display_items = displayMatch[1];

      html += `<div class="collector" data-path="${collectorPath}" data-opts='${JSON.stringify(collectorOpts)}'></div>`;
      i++;
      continue;
    }
